...
const TOKEN = process.env.PRINTFUL_TOKEN || ''; // pas requis pour /catalog, mais accepté
...
async function listCatalogProductIds(){
  ...
  let total=Infinity;
  const ids=[];
  console.log(`Images v2 (catalog) — PAGE_LIMIT=${PAGE_LIMIT}, INIT_CONC=${CONC}`);
