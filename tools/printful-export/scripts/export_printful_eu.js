...
// Load .env locally (ignored in git). In Actions, PRINTFUL_TOKEN comes from secrets.
...
if (!TOKEN) throw new Error('PRINTFUL_TOKEN missing. Put it in .env locally, or define it as a GitHub Secret.');

const argv = process.argv.slice(2);
function arg(name, def = null){ const i=argv.indexOf(`--${name}`); return i>=0 ? (argv[i+1] ?? true) : def; }

const REGION   = arg('region', 'europe');     // selling_region_name
const DEST     = arg('dest', null);           // destination_country (ex: FR)
const MAX      = Number(arg('max', 200));     // cap to keep runs fast
const OUTDIR   = arg('out', 'printful_export');
const WANT_ZIP = argv.includes('--zip');      // needs jszip
...
async function getJSON(url, { retry=5, backoff=800 } = {}){
  for (let a=1; a<=retry; a++){
    let r=null;
    try { r = await fetch(url, { headers:{ Authorization:`Bearer ${TOKEN}` } }); } catch(_){}
    if(!r){
    ...
    if (r.status===429 || (r.status>=500 && r.status<=599)){
      ...
      if(a===retry) { const t=await r.text().catch(()=> ''); throw new Error(`HTTP ${r.status} ${url} :: ${t}`); }
...
async function listAllProducts({ region, dest, max=200 }){
  ...
  while(true){
    ...
    for (const it of list){ all.push(it); if(max && all.length>=max) return all; }
    offset += list.length;
...
async function main(){
  ...
  const productsRows=[], variantsRows=[], categoriesRows=[], productCategoriesRows=[];
  const productImagesRows=[], variantImagesRows=[], sizesRows=[], pricesRows=[], availabilityRows=[], countriesRows=[];
  ...
  }catch(e){ console.warn('countries skipped:', e.message); }
  ...
  for (const p of base){
    ...
    let pd=p;
    try{ const r=await getJSON(`${API_BASE}/v2/catalog-products/${pid}`); pd=r?.result||r?.data||r||pd; }catch(e){ console.warn(`detail #${pid}:`, e.message); }
    ...
    // product prices (if provided)
    try{
      const prUrl = pd?._links?.product_prices?.href;
      if(prUrl){
        const u = new URL(prUrl); if (REGION) u.searchParams.set('region', REGION);
    ...
    try{
      ...
      vlist = (vj?.result || vj?.data || vj || []); vlist = vlist.items || vlist || [];
    }catch(_){ vlist=[]; }
    ...
    for (const v of vlist){
      ...
      try{
        ...
        const arr = items?.items || items; const list = Array.isArray(arr)?arr:[arr];
        for (const a of list){
  ...
  // write CSV
  writeCSV('products.csv', productsRows, ['product_id','main_category_id','type','name','brand','model','image_hero','variant_count','is_discontinued','description','sizes_list','colors_list','techniques','placements_schema','product_options','_links_self','_links_variants','_links_categories','_links_prices','_links_sizes','_links_images','_links_availability']);
  writeCSV('variants.csv', variantsRows, ['catalog_variant_id','catalog_product_id','name','size','color','color_code','color_code2','image_main','placement_dimensions','_links_self','_links_variant_prices','_links_variant_images','_links_variant_availability']);
  ...
  if (WANT_ZIP){
    ...
    for (const f of files){ zip.file(f, fs.readFileSync(path.join(OUTDIR, f), 'utf8')); }
